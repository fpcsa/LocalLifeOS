...
function BulkRescheduleForm({ open, onClose, tasks, timezone }: { open: boolean; onClose: () => void; tasks: Task[]; timezone: string }) {
  const queryClient = useQueryClient();
  const pushToast = useUiStore((state) => state.pushToast);
  const { register, handleSubmit, reset, formState: { errors } } = useForm<{ start: string; end: string }>();
  const reschedule = useMutation({ mutationFn: (values: { start: string; end: string }) => { const start = fromDateTimeLocal(values.start, timezone); const end = fromDateTimeLocal(values.end, timezone); if (!start || !end || new Date(end) <= new Date(start)) throw new Error("The end must be after the start."); return bulkRescheduleTasks(tasks.map((task) => ({ id: task.id, revision: task.revision, scheduled_start_at: start, scheduled_end_at: end }))); }, onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: queryKeys.tasks.all }); pushToast({ title: "Tasks rescheduled", tone: "success" }); reset(); onClose(); }, onError: (error) => pushToast({ title: "Couldn't reschedule tasks", description: error instanceof Error ? error.message : "Reload and try again.", tone: "error" }) });
...
export function TasksWorkspace() {
  ...
  if (tasks.isLoading || projects.isLoading || preferences.isLoading) return <SkeletonList rows={8} />;
  if (tasks.isError || projects.isError || preferences.isError || !preferences.data) return <ErrorState retry={() => { void tasks.refetch(); void projects.refetch(); void preferences.refetch(); }} />;
  ...
  return (
    <div className="space-y-6">
      ...
      <TaskForm onClose={() => setCreateOpen(false)} open={createOpen} projects={(projects.data?.data || []) as Project[]} tasks={tasks.data?.data || []} timezone={timezone} />
      <ProjectForm onClose={() => setProjectOpen(false)} open={projectOpen} />
      <BulkRescheduleForm onClose={() => setRescheduleOpen(false)} open={rescheduleOpen} tasks={(tasks.data?.data || []).filter((task) => selected.has(task.id))} timezone={timezone} />
