...
import { currentMonthDates, localDateKey } from "@/lib/date-range";
...
function monthRange(timezone: string, now = new Date()): { reportStart: string; startDate: string; endDate: string } {
  const dates = currentMonthDates(timezone, now);
  const [year, month] = dates.startDate.split("-").map(Number);
  return {
    reportStart: new Date(Date.UTC(year, month - 6, 1)).toISOString().slice(0, 10),
    ...dates,
  };
}
...
export function FinanceWorkspace() {
  ...
  const committed = useQuery({ queryKey: queryKeys.finance.committed(range.endDate), queryFn: () => getCommittedBalance(localDateKey(new Date(), timezone), range.endDate) });
  ...
  const required = [preferences, accounts, transactions, categories, budgets, cashFlow, spending, committed, subscriptions, savings];
  ...
  return (
    <div className="space-y-6">
      ...
      <FinanceForms accounts={accounts.data?.data || []} categories={categories.data?.data || []} close={() => setForm(null)} form={form} timezone={timezone} />
    </div>
  );
}
